...
<body style="text-align: center;">
	...
	<form action="${pageContext.servletContext.contextPath }/log">
		<div style="border-bottom-color: red;" align="center">
			<input type="date" name="begin" value="${param.begin }" />~ <input
				style="border-bottom: thick;" type="date" name="end"
				value="${param.end }" />
	...
	<form action="${pageContext.servletContext.contextPath }/delete">
		<table border="1" style="width: 1000px">
			...
			<tr>
				<c:forEach var="one" items="${logs}">
					...
					<td>${one.spendAt }</td>
					<td>${one.useDesc }</td>
					<td><fmt:formatNumber value="${one.amt }" pattern="#,###" /></td>
		...
		<div>
			...
			<a href="${pageContext.servletContext.contextPath }/writeComplex"><button>기록추가</button>
			</a>
		</div>
